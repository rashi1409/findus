...
router.post('/addlocation', function(req, res, next) {
  ...
  var file1nm=new Date().getTime()+file1.name
  ...
  file1.mv(file1des)
	...
	if(req.files.file2!=undefined)
	{
	  ...
	  var file2nm=new Date().getTime()+file2.name
	...
	if(req.files.file3!=undefined)
	{
	  ...
	  var file3nm=new Date().getTime()+file3.name
	...
	if(req.files.file4!=undefined)
	{
	  ...
	  var file4nm=new Date().getTime()+file4.name
...
router.get('/viewlocations',function(req, res, next) {
  userModel.fetchlocationuser(req.session.sunm).then((result)=>{
  	res.render("viewlocationsuser",{'sunm':req.session.sunm,'location_list':result})
  }).catch((err)=>{	
  	console.log(err)
  })
})
